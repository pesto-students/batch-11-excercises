...
const trimUnrated = async (db) => db.collection(movieDetails).find({
  $and: [{ rated: { $exists: true } }, { rated: { $ne: 'UNRATED' } }],
}).count();

/* Q9 (*)
  Return number of movies in which "tomato" field exists but "tomato.rating" does not
*/
const unratedByTomato = async (db) => db.collection(movieDetails).find({
...
/* Q10 (*)
  ...
  metacritic >= 90
*/
const goodMovies = async (db) => db.collection(movieDetails).find({
...
*/
const arrayAll = async (db) => db.collection(movieDetails).find({
...
*/
const fieldArraySize = async (db) => db.collection(movieDetails).find({
  countries: { $size: 4 },
}).count();
...
*/
const addField = async (db) => db.collection(movieDetails).findOneAndUpdate({
  title: 'Iron Man 3',
}, { $set: { myRating: 88 } });
...
*/
const incrementalUpdate = async (db) => db.collection(movieDetails).findOneAndUpdate({
  title: 'Gone Girl',
}, { $inc: { metacritic: 5 } });
