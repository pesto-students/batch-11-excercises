...
describe('Promise.each', () => {
  ...
  it('waits for returned thenable before proceeding next', () => {
    const b = [1, 2, 3];
    const a = [pushToArray(1), pushToArray(2), pushToArray(3)];
    return Promise.resolve(a).each((val) => {
      b.push(val * 50);
      return pushToArray(val * 500, b);
    }).then(() => {
      assert.deepEqual(b, [1, 2, 3, 50, 500, 100, 1000, 150, 1500]);
    });
  });

  it("doesn't iterate with an empty array", () => Promise.each([], () => {
    throw new Error();
  }).then((ret) => {
...
describe('Promise.prototype.each', () => {
  ...
  it("doesn't iterate with an empty array", () => Promise.resolve([]).each(() => {
    throw new Error();
  }).then((ret) => {
    assert.deepEqual(ret, []);
  }));
