...
module.exports = {
  ...
  testBasicAuth(test) {
    server = http.createServer((req, res) => {
      ...
      axios.get(`http://${user}@localhost:4444/`, { headers }).then((res) => {
        const base64 = Buffer.allocUnsafe(`${user}:`, 'utf8').toString('base64');
  ...
  testBasicAuthWithHeader(test) {
    server = http.createServer((req, res) => {
      ...
      axios.get('http://localhost:4444/', { auth, headers }).then((res) => {
        const base64 = Buffer.allocUnsafe('foo:bar', 'utf8').toString('base64');
        test.equal(res.data, `Basic ${base64}`);
        test.done();
      });
    });
  ...
  testMaxContentLength(test) {
    ...
    server = http.createServer((req, res) => {
      ...
      let success = false; let failure = false; let
        error;
  ...
  testFailedStream(test) {
    server = http.createServer((req, res) => {
      ...
      axios.post('http://localhost:4444/',
        fs.createReadStream('/does/not/exist')).then(() => {
        test.fail();
      }).catch((err) => {
    ...
    server = http.createServer((req, res) => {
      ...
      proxy = http.createServer((request, response) => {
        ...
        axios.get('http://localhost:4444/', {
          ...
          const base64 = Buffer.allocUnsafe('user:pass', 'utf8').toString('base64');
